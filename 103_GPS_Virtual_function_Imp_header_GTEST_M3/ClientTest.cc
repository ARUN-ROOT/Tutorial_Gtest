...
TEST(testsuit, subtest_ClientNonVirtualFun_Inside_class) {  
    std::cout << "Test started" << std::endl;
    MockGPS pMockGPS;
    Client obj(&pMockGPS); // passing address
    std::cout << "Before clientNonVirtual" << std::endl;
...
TEST(testsuit, subtest_clientNonVirtualFunSpecial) {  
    MockGPS pMockGPS;
    Client obj(&pMockGPS); // passing address
    int result = obj.clientNonVirtualFunSpecial();
    std::cout << " clientNonVirtualFunSpecial in Gtest " << std::endl;
     EXPECT_EQ(1,result);
